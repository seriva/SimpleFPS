			...
			switch (block) {
				case 1:
					if (i + 1 < outer && data[i + 1][j] > 1) {
						...
						entity.addChild(
							new PointLightEntity(
								[i * cellSize + 1.85, 0.25, j * cellSize - 1.25],
								2,
								wallLight,
								1.85,
							),
					...
					if (j + 1 < inner && data[i][j + 1] > 1) {
						entity = new MeshEntity(
							[i * cellSize, 0, j * cellSize + 1.505],
							meshMap.get(1),
						);
						entity.addChild(
							new PointLightEntity(
								[i * cellSize - 1.25, 0.25, j * cellSize + 1.85],
								2,
								wallLight,
								1.85,
							),
				...
				case 2:
					...
					Scene.addEntities(
						new MeshEntity([i * cellSize, 0, j * cellSize], meshMap.get(4)),
					);
					break;
				case 3:
					Scene.addEntities(
						new MeshEntity([i * cellSize, 0, j * cellSize], meshMap.get(3)),
					);
					Scene.addEntities(
						new MeshEntity([i * cellSize, 0, j * cellSize], meshMap.get(5)),
					);
					Scene.addEntities(
						new MeshEntity([i * cellSize, 0, j * cellSize], meshMap.get(6)),
					);
					Scene.addEntities(
						new PointLightEntity(
							[i * cellSize, 3, j * cellSize],
							8,
							wallLight,
							1.0,
						),
...
const load = async (name) => {
	...
	const response = await Utils.fetch(
		`${window.location}resources/maps/${name}`,
	);
